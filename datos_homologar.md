...
Cada vez que un equipo envía por TCP **9912** una trama POLLO de datos (campos `d01`…`d05` con payloads `82A7xx`), el sistema debe:
...
```
Dispositivo ──TCP:9912──► tcp_bridge ──► MongoDB (todas las tramas)
                              │
...
|------|----------------|-----------------|
| Cabecera | `CONNECT ip:port → 9912` | No |
...
Ejecutar **después** de persistir en MongoDB (no bloquear el socket 9912).
